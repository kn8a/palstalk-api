...
const sendFriendRequest = asyncHandler(async (req, res) => {
	...
	const requestExists = await FriendRequest.find({
		from: req.user._id,
	...
	const reverseRequestExists = await FriendRequest.find({
	...
	const updateSender = await User.findByIdAndUpdate(from._id, {
		$push: { pending_requests: newRequest._id },
	})
	const updateReceiver = await User.findByIdAndUpdate(to._id, {
		$push: { pending_requests: newRequest._id },
	})
...
const getReceived = asyncHandler(async (req, res) => {
	const requests = await FriendRequest.find({
		...
		path: "from",
		select: { name_first: 1, name_last: 1, profile_pic: 1 },
	})
...
const getRequest = asyncHandler(async (req, res) => {
	...
	if (req.user.id != receiver._id || req.user.id != sender._id) {
		//if authenticated user is not receiver nor sender
		res.status(400).json({
...
const acceptFriendRequest = asyncHandler(async (req, res) => {
	...
	const sender = request.from
	const receiver = request.to
	if (req.user.id != receiver._id) {
		//if receiver is not the authenticated user
	...
	const updateSender = await User.findByIdAndUpdate(sender._id, {
		$push: { friends: receiver },
		$pull: { pending_requests: request._id },
	})
	const updateReceiver = await User.findByIdAndUpdate(receiver._id, {
		$push: { friends: sender },
		$pull: { pending_requests: request._id },
	})
...
const cancelFriendRequest = asyncHandler(async (req, res) => {
	...
	const sender = request.from
	const receiver = request.to
	if (req.user.id != sender._id) {
		//if sender is not the authenticated user
		res.status(400).json({ message: "Unauthorized action" })
		return
	}
...
const unfriendById = asyncHandler(async (req, res) => {
	...
	if (userIsFriend == -1) {
		res.status(400).json({
			message: "Can't unfriend someone who is not your friend",
		})
	} else {
		const updateMe = await User.findByIdAndUpdate(req.user._id, {
			$pull: { friends: userToUnfriend },
		})
		const updateThem = await User.findByIdAndUpdate(userToUnfriend, {
