...
const createPost = asyncHandler(async (req, res) => {
  ...
  const updateAuthor = await User.findByIdAndUpdate(req.user._id, {
...
const reportPost = asyncHandler(async (req, res) => {
  const post = await Post.findById(req.params.postId)
  const alreadyReported = await post.reports.findIndex(
    (id) => id.toString() == req.user._id
  )
...
const getBoard = asyncHandler(async (req, res) => {
  ...
  const boardPosts = await Post.find({
    author: { $in: meAndFriends },
    is_reported: false,
  })
    .populate("comments")
    .populate({
